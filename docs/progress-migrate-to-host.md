...
### M3 — Windows wrapper + 文件

- 新增 `scripts/migrate_to_host.ps1`：偵測 `wsl.exe`、用 `wslpath` 把 repo 路徑
  轉成 WSL 路徑，再 `wsl bash -lc "cd ... && ./scripts/migrate_to_host.sh <args>"`
  原樣轉發所有參數。Windows 端不需另裝 rsync/ssh。
- README 新增「一鍵搬到另一台主機（migrate）」段落：cp config → dry-run →
  `--apply --verify` → `--verify-only`，含 `--bwlimit` / `--no-delete` / Windows 用法。
- 修掉 `-h` 會多印幾行程式碼的小 bug（sed 固定行號 → awk 抓連續註解行）。

## 完成狀態

3 個 milestone 全部完成。一鍵指令：

```bash
cp scripts/migrate.conf.example scripts/migrate.conf   # 填目標主機
./scripts/migrate_to_host.sh --apply --verify          # 搬 + 驗
```
