...
4. **狀態：背景跑中**。
   ...
   - Wakeup poll @ 12:56（+2h 14min）：**TaiwanStockPrice 完成於 12:39:26**（共 ~117 min，2,744 done + 344 empty）；TaiwanStockPriceAdj 已起跑 ~17 min，257 done + 31 empty = 9%，max date 已推到 2026-05-22；按相同 rate 全部完成 ETA **~14:36**。
   - 用戶要求 30min cadence 定期回報 — 改 wakeup +30min
   - Wakeup poll @ 13:28（+49 min from Adj start）：1,146 done + 146 empty = 41.8%，rate ~26/min 穩定，ETA 14:36 一致。reschedule +30min。
   - Wakeup poll @ 13:55（+1h 16min from Adj start）：65.4% (1,785+234)，rate 穩定 ~26/min；ETA 14:36 一致。reschedule +30min。
   - Wakeup poll @ 14:28（+1h 49min from Adj start）：93.5% (2,569+319)，剩 200 檔 ~8min；ETA 14:36 一致。reschedule +15min 抓完成時刻。

   - **完成 @ 14:36:17 TPE**（log 明確標記）；總時長 117 min for Adj，跟 TaiwanStockPrice 一樣。整個 daily refresh = 3h 54min 從 10:42 launch 到 14:36 結束。
   - **M3 收尾流程（@ 14:48-14:51 TPE）**：
     - 釋 catalog 寫鎖（kill PID 311814，閒置 `duckdb -ui` 4h 40min）
     - `cp` snapshot 2.6 GB sqlite → `bronze/finmind/finmind_2026-05-25.sqlite`（3.8s, reflink）
     - sha256: `06ba03a08b0ebed64079c383ebb1d3918c3cee0e25d937f88653f662a8eea27c`
     - `scripts/restore_finmind_views.py` glob 自動 rebind 9 個 view 到 new sqlite（quant.duckdb + quant_public.duckdb 各一次）
     - 驗證：`finmind_stock_price_norm` 10,592,556 列 / max **2026-05-22** (was 5/15)；`finmind_stock_price_adj_norm` 10,588,095 列 / max **2026-05-22** (was 5/13)
     - tick crawler 重啟 PID 325570，繼續 5/15-5/22 8 天 × 3088 檔（從上次斷點接續）
     - gap_report.py 重生 dashboard：summary 不變（OK=12 / WARN=1 / STALE=8 / EMPTY=1 / INFO=4）但 finmind views 的 max date 已更新到 5/22
