...
QUANTDATA medallion 架構裡，silver 達到 100% 完整度後不是終點——還要進一步衍生成 gold factor / 物化 parquet，下游策略才能直接 `read_parquet()`。本頁說明把這流程自動化的兩支工具：**audit script** 偵測 ripe candidates；**agent** 把 audit + builder + registry + dashboard 串起來。

## 什麼算 "ripe candidate"？

凡是同時滿足下面 3 條的 catalog view：

1. **Severity = OK**（trading-day-aware lag ≤ 1 天，dashboard 上顯示 100% 完整度）
2. **`scripts/gap_report.py` 的 DATASETS registry 裡 `gold_paths` 為空 tuple**（亦即還沒 backlink 任何 gold parquet）
3. 在 catalog 中可 `DESCRIBE` 出 schema（也就是 view 在 catalog 中存在、不是 ghost）

例外：INFO tier（snapshot / 純對帳 view）只要 `abs(lag_days) ≤ 1` 也算 100%。
...
當前狀態（截至 2026-05-25 evening）：

```
✅ goldify_audit: no 100%-complete views are missing gold. Catalog is fully goldified.
```

### 2. Agent 觸發 — 偵測到 candidate 就跑 routine

對 Claude Code 說任何下面這類話，自動 route 到 `goldify-100pct` agent：
